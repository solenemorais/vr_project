Shader "Custom/SolarSystem/ToonPlanet"
{
	Properties
	{
		//Noise
	...
	SubShader
	{
		Pass
		{
			...
			v2f vert (appdata v)
			{
				...
				//Noise
				o.srcPos = v.vertex.xyz;
				o.srcPos *= _NoiseFreq;
				o.srcPos.x *= _NoiseAspect;
				o.srcPos.y /= _NoiseAspect;
			...
			float4 frag (v2f i) : SV_Target
			{
				...
				float3 lightVector = normalize(float3(unity_4LightPosX0.x+1,unity_4LightPosY0.x,unity_4LightPosZ0.x)-i.worldPos);
				//float3 lightVector = unity_LightPosition[0].xyz;
				...
				float4 light = lightIntensity * fixed4(1,1,1,1);
				...
				return (light + _AmbientColor + specular + rim) * col;
			}
			ENDCG
		}
